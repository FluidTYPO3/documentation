...
When the checks all pass and the commit is amended, two things can happen:

1. If your working branch has not yet been published then continue to the next chapter.
2. If your working branch has already been published: call `git push -f` to forcefully update the published branch with your amended changes, then continue to the next chapter.
